...
import { FORK_REPO, fetchLatestForkVersion } from "./fork";
 ...
 * ({@link FORK_REPO}), not the npm registry upstream consults.
 *
 ...
 * The upstream `channel` option is accepted but ignored: the fork publishes
 * a single release channel.
 */
export async function getLatestRelease(
	...
	let version: string;
	try {
		version = await fetchLatestForkVersion(timeoutMs);
	} catch (err) {
		if (isTimeoutError(err)) {
